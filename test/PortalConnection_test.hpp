...
#define SUITE_NAME Portal_Connection_Test
...
// Checkin - Failed
TEST(SUITE_NAME, Checkin_Failed) {
...
// Checkin - Garbage
TEST(SUITE_NAME, Checkin_Garbage) {
    String payload("{\"succdlfkjfdsa;vm44{}{%1677833210}");
    auto result = PortalConnection::decodeHashOfAuthedCards(payload);
    ASSERT_FALSE(result.has_value());
}
TEST(SUITE_NAME, Checkin_Malformed_JSON) {
...
TEST(SUITE_NAME, Auth_Single_Card_Good) {
...
TEST(SUITE_NAME, Auth_Single_Card_Failed) {
...
TEST(SUITE_NAME, Auth_Single_Card_Missing_Access_JSON) {
...
TEST(SUITE_NAME, Auth_Single_Card_Garbage) {
    String payload("acc85ib32g98v n3n9v983nonv9oin{3[21}$$11]}");
    auto result = PortalConnection::decodeCardAuth(payload);
    ASSERT_FALSE(result.has_value());
}
TEST(SUITE_NAME, Auth_Single_Card_Malformed_JSON) {
...
// All cards - Good
TEST(SUITE_NAME, All_Card_Auth_Good) {
...
// Should not decode.
TEST(SUITE_NAME, All_Card_Auth_Too_Many_Cards) {
    // 255 numbers.
...
// All cards - Garbage
TEST(SUITE_NAME, All_Card_Auth_Garbage) {
...
TEST(SUITE_NAME, All_Card_Auth_Malformed_JSON) {
